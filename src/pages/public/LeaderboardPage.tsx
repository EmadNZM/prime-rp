...
import { Trophy, Clock, Wallet, Shield, Database, RefreshCw, Users, Wifi, AlertCircle, Medal } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-[#E5E5E5] pt-28 pb-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="p-5 rounded-3xl bg-[#0D0D0F] border border-[#222226] mb-8 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
          ...
          <div className="flex items-center gap-2 text-xs font-bold text-amber-400 bg-amber-400/10 px-3.5 py-1.5 rounded-full border border-amber-400/20 shrink-0">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-8">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${
              activeTab === 'playtime'
                ? 'bg-[#151518] text-white border border-[#C8874B]/60'
                : 'bg-[#0D0D0F] text-[#777] hover:text-[#AAA] border border-[#222226]'
            }`}
          >
            <Clock className="w-3.5 h-3.5" />
            <span>{language === 'ar' ? 'ساعات اللعب' : 'Playtime'}</span>
            <span className="px-1.5 py-0.2 text-[9px] rounded-md bg-amber-500/15 text-amber-400 border border-amber-500/25">
        ...
        {isLoading ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-4 px-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-[#C8874B] hover:text-[#DF9F64] transition-colors cursor-pointer"
              >
            ...
            {livePlayers.length === 0 ? (
              <div className="text-center py-16 px-6 rounded-3xl bg-[#0D0D0F] border border-[#222226]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {livePlayers.map((player) => (
                  <div
                    key={player.id}
                    className="p-3.5 rounded-2xl bg-[#0D0D0F] border border-[#222226] flex items-center justify-between hover:border-[#C8874B]/40 transition-all card-hover-lift group"
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-8 h-8 rounded-xl bg-[#151518] border border-[#252528] flex items-center justify-center text-xs font-mono font-black text-[#C8874B]">
                        #{player.id}
                      </span>
          ...
          <div className="text-center py-20 px-6 rounded-3xl bg-[#0D0D0F] border border-[#222226] max-w-xl mx-auto shadow-2xl">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#151518] border border-[#252528] text-xs text-[#AAA]">
          ...
          <div className="space-y-3">
            {leaderboardData.map((item) => (
              <div
                key={item.rank}
                className="p-4 sm:p-5 rounded-2xl bg-[#0D0D0F] border border-[#222226] hover:border-[#C8874B]/40 transition-all flex items-center justify-between card-hover-lift"
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#151518] border border-[#252528] flex items-center justify-center text-xs font-black text-[#C8874B]">
                    #{item.rank}
                  </div>
                  <div>
